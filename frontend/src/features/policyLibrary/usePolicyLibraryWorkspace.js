...
export function usePolicyLibraryWorkspace() {
  ...
  useEffect(() => {
    void load({ category, date, page, pageSize });
  }, [category, date, page, pageSize]);

  async function load(filters = { category, date, page, pageSize }) {
    setLoading(true);
    try {
  ...
  async function importFile(file) {
    ...
    try {
      ...
      await load({ category, date, page: 1, pageSize });
    } catch (err) {
      toast.error(err.message);
    } finally {
      setImporting(false);
  ...
  async function deletePolicy(item) {
    ...
    try {
      ...
      await load({ category, date, page: nextPage, pageSize });
    } catch (err) {
      toast.error(err.message);
    } finally {
      setDeletingPolicyId('');
