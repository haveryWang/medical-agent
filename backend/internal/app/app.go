...
func New(ctx context.Context, cfg config.Config, logger *log.Logger) (*App, error) {
	...
	modelConfigResolver := func(callCtx context.Context) models.ModelConfig {
		modelConfig, err := mongoStore.GetModelConfig(callCtx, cfg)
		if err != nil {
			logger.Printf("读取模型配置失败，使用环境变量兜底: %v", err)
			return models.ModelConfig{
				...
				QwenEmbeddingAPIKey:    cfg.QwenEmbeddingAPIKey,
	...
	qwenClient := qwen.New(cfg, qwen.WithModelConfigResolver(modelConfigResolver))
	vectorClient := vector.New(cfg)
	deepSeekClient := deepseek.New(cfg, deepseek.WithModelConfigResolver(modelConfigResolver))
